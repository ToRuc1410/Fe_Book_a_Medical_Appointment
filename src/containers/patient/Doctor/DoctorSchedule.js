...
class DoctorSchedule extends Component {
    ...
    async componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (prevProps.doctorFromParent !== this.props.doctorFromParent) {
            let allDays = this.getArrDays(this.props.language);
            let res = await getScheduleByDate(this.props.doctorFromParent, allDays[0].value);
            this.setState({
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <div className='col-4'>
                    <select className="text-capitalize p-1 text-primary"
                        style={{ border: "none", borderBottom: "1px solid grey", outline: "none" }}
                    ...
                    <div className="justify-content-center" >
                        {allAvalableTime && allAvalableTime.length > 0 ?
                            <>
                                {allAvalableTime.map((item, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            style={{ width: "140px", color: "blue" }}
                                            onClick={() => this.handleClickOpenScheduleTime(item)}
                                        >
                                            {timeDisplay}
                                        </button>
                                    )
                                ...
                                <p><FormattedMessage id={"patient.detail-doctor.choose"} /> <i className="fas fa-hand-point-up"></i> <FormattedMessage id={"patient.detail-doctor.book-free"} /> </p>
                            </>
                            :
                            <p><FormattedMessage id={"patient.detail-doctor.non-schedule"} /> </p>
                        }
                    </div>
                </div>
                <BookingModal
